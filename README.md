...
- 여행 상품 목록, 상세, 등록, 수정
- 상품 검색, 예약 가능/마감 탭, 페이지 이동, 최근 본 상품
- 트립토크 게시판 목록, 상세, 작성, 수정
- 로그인 기반 인증 흐름과 accessToken 상태 관리
- 상품 문의, 답글, 찜, 구매, 포인트 충전 등 사용자 상호작용
...
- 상품 목록은 서버 초기 데이터와 클라이언트 상호작용을 분리하고, 검색/탭/페이지 이동은 API 기반으로, 카테고리 필터와 최근 본 상품은 클라이언트 상태 기반으로 정리했습니다.
- 상품 상세는 인증 상태를 확인한 뒤 클라이언트 hook을 실행하도록 분리하고, 판매자/문의 작성자 권한, 이미지 fallback, Kakao Map 표시 조건, 문의/답글 흐름을 점검했습니다.
- 찜은 optimistic UI와 Apollo cache 업데이트를 사용하고, 구매/포인트 충전은 PortOne 결제 결과와 유저 포인트 refetch 흐름을 연결했습니다.
