이제부터는 여러분이 직접 만들어보는 시간입니다! 지금까지 배웠던 내용들을 잘 생각해보면서, 각자 **컴포넌트를 만들고 재사용성을 고려하여** 기능을 구현해보세요. 이제는 더 이상 세세하게 가이드를 제공하지 않겠습니다. 여러분만의 스타일로 코드를 작성하고, 기능을 완성해가는 과정을 통해 **자기만의 방법을 찾아가시길** 바랍니다.

- [ ] 완성된 `homework35`폴더를 활용하여 `homework36`을 완성해 주세요.
- [ ] opengraph를 적용해주세요.
  1. 여행상품 상세페이지: dynamic opengraph 적용
  2. 전역 설정(나머지 모든 페이지): static opengraph 적용
- [ ] 남은 시간에는 못했던 기능을 마저 완성해 주시거나, 리팩토링을 진행해주세요.
